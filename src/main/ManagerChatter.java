...
public class ManagerChatter extends JFrame implements Runnable {
	...
	public ManagerChatter() {
		...
		txtServerPort.setFont(new Font("Segoe Print", Font.BOLD, 15));
		panel.add(txtServerPort);

		txtPort = new JTextField();
		txtPort.setFont(new Font("Segoe Print", Font.BOLD, 15));
		...
		txt_SName.setFont(new Font("Segoe Print", Font.BOLD, 15));
		panel.add(txt_SName);
		...
		while (true) {
			try {
				...
				if (aSocket != null) {
					...
					int post = s.indexOf(":");
					String receiver= s.substring(post + 1);
					ChatPanel p = new ChatPanel(aSocket, this.sName, receiver);
					tabbedPane.add("<html><font color='red'>"+receiver+"</font></html>", p);
					p.updateUI();
				}
				Thread.sleep(100);
			} catch (Exception e) {
